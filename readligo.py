...
def loaddata(filename, ifo=None, tvec=True, readstrain=True, strain_chan=None, dq_chan=None, inj_chan=None, use_torch=False):
    """
    ...
    if use_torch:
        if tvec:
            return torch.from_numpy(strain), torch.from_numpy(time), channel_dict
        else:
            return torch.from_numpy(strain), torch.from_numpy(meta), channel_dict
